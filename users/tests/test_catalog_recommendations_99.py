...
class TestSalonStateGatesThePool:
    ...
    def test_master_without_a_salon_does_not_500_the_endpoint(
        ...
        orphan = _make_specialist(
            tenant_new, suffix="0602", name="Мастер без салона",
        )
        # Ребро держит только мастер салона услуги, поэтому мастер сначала
        # в салоне услуги, а салон у него снимается мимо clean(). Ровно так
        # мастер без салона и возникает в данных: бэкфилл DRF-242.4 не закрыт.
        # Предмет теста — мастер без салона, и он таким и остаётся.
