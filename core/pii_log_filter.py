...
#
# Цена, названная числом: в самом дереве ложных срабатываний 0 на 108 188
# проверенных строк. Снаружи — 11 испорченных строк на 30 придуманных
# диагностических (было 10 до этого листа): `max:<небольшое число>`
# (`pool stats min:2 max:10` → `min:2 [IDENTITY]`), `wa:session:cache`,
# `bot:8000`. Это порча ДИАГНОСТИКИ, а не потеря охраны. Сужать по длине
# сегмента нельзя: есть живые идентификаторы в две буквы (`bot:c1`).
...
_IDENTITY_RE: Final[re.Pattern[str]] = re.compile(
    ...
    #
    # Ложных срабатываний это НЕ уменьшает, а слегка добавляет — замер:
    # `POOL STATS MIN:2 MAX:10` и `DRF-2020: Max:12 comments` теперь тоже
    # портятся. Сделка та же, что во всём листе: закрыть дыру в охране ценой
    # шума в диагностике, а не наоборот.
    re.IGNORECASE,
)
