...
# золотых узлах.
#
# В списке ровно то, что каталог НОСИТ, а не то, что мог бы. Замер по дереву
# (форма `слово:слово`, вне тестов / в тестах и доках): `bot` — 41 / 638,
# `max` — 10 / 11, `telegram` — 0 / 2. Прежняя редакция везла ещё `tg`, `vk`,
# `viber`, `whatsapp`, `wa` — у всех пяти замер дал 0 / 0, то есть они не
# защищали ничего, а ложные срабатывания несли (`HGETALL wa:session:cache`).
# `telegram` оставлен не «на будущее»: канал настоящий, в дереве он есть внутри
# `bot:telegram:<id>`, и бареформа — одно сообщение отсюда.
#
# Известная цена закрытого списка, названная числом: в самом дереве ложных
# срабатываний 0, но `max:<небольшое число>` — настоящая ловушка снаружи
# (`pool stats min:2 max:10` станет `min:2 [IDENTITY]`). Это порча
# ДИАГНОСТИКИ, а не потеря охраны, и сужать по длине сегмента нельзя: у нас
# есть живые идентификаторы в две буквы (`bot:c1`).
#
# Список пополняется, когда в дереве появляется новый источник; за этим следит
# `TestEveryIdentityTheCatalogCarries` — перепись по вокабуляру продукта, а не
# по соседнему шаблону.
_IDENTITY_SOURCES: Final[str] = "bot|max|telegram"
