...
class TestPlacesGeocodingReadiness:
    ...
    def test_the_message_names_what_to_fill(self, unconfigured):
        ...
        **Названный предел этого узла.** Он держит литерал ``NOMINATIM_BASE_URL``,
        а в нём есть подстрока-маркер закрытого списка геокодеров. В рабочем
        модуле (`core/geocoding/checks.py`) таких литералов нет намеренно —
        сторож ``test_the_set_of_modules_that_talk_to_geocoders_is_closed``
        записал бы его в «ходящие к геокодерам». Здесь литерал допустим по двум
        независимым причинам, и обе — свойства сторожа, а не везение:

        1. он пропускает тесты: ``if "/tests/" in rel … continue``;
        2. сравнение регистрозависимое (``marker in text``), а маркер — голое
           ``nominatim`` в нижнем регистре.

        Адрес, на котором это стоит: ``core/tests/test_geocoding_contract.py:264-288``
        (узел ``test_the_set_of_modules_that_talk_to_geocoders_is_closed``).
        Названо адресом, а не именем, намеренно: кто снимет исключение
        ``tests/``, увидит связь отсюда сразу, а не через красный прогон.
        Независимость двух причин тоже важна — снятие исключения оставляет
        регистр, и наоборот.

        Если этот файл однажды переедет из ``tests/`` — первая причина исчезнет,
        и узел придётся переписать на динамическое имя настройки от провайдера.
        """
        message = _run_places()[0].msg
