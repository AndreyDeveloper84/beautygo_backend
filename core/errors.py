...
class ErrorCode(str, Enum):
    ...
    # DRF-2085 — привязка администратора салона из админки бота отказала:
    # салона нет/неактивен, MAX-личность уже связана или под ней настоящая
    # учётка, ключ идемпотентности переиспользован с другим телом, либо
    ...
    # его одним except. HTTP 423 Locked — не 403 («нельзя вам») и не 404
    # («нет такого»): ресурс заперт по воле его владельца, временно.
