...
``0024`` из seed. Пары (подкатегория, название) команда **не использует**:
...
def template_for_code(code: str) -> tuple[ServiceTemplate | None, str]:
    ...
    ``>1`` невозможно; ``0`` — bootstrap 0024 на этой базе не прошёл либо
    ...
    if tpl is None:
        return None, (f"канон с canonical_code={code} не найден в базе "
                      "(bootstrap 0024 не прошёл или код не из справочника)")
    return tpl, ""
