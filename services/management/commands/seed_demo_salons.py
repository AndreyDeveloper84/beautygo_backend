...
до осознанного включения. Проверено тестом
``test_inactive_tenant_alone_does_not_hide_masters``: **одного**
``Tenant.is_active=False`` для этого НЕ достаточно.
``RecommendationEngine`` фильтрует ``SpecialistProfile`` и к таблице
тенантов не присоединяется вовсе, поэтому мастер выключенного тенанта
попадает в выдачу как ни в чём не бывало. Менеджер ``Tenant.objects``
прячет строку от того, кто спрашивает про тенанты, — а подбор про
тенанты не спрашивает.

Поэтому демо-данные заперты тремя независимыми замками:
...
Замки 2 и 3 — те, что реально держат. ``--activate`` снимает все три
разом и не создаёт ничего: включение — отдельное решение владельца, и
оно должно быть одним явным действием, а не побочным эффектом сида.
