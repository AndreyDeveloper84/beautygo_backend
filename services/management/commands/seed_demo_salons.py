...
пилот ``formula-tela`` — там настоящий прайс настоящего салона; почему
защищён именно он и почему не из-за людей — у ``PROTECTED_SLUGS``).
...
#
# Что защищается — ПРАЙС, а не люди. У «Формулы тела» настоящий прайс
# настоящего салона: 58 услуг с ценами и длительностями, которые салон дал
# нам сам (число — из решения владельца 25.09). Засев демо-данными его
# затрёт, и это необратимо.
#
# Клиенты в системе при этом тестовые — владелец 25.09 дословно: «в Формуле
# тела нет настоящих людей». Прежде здесь и в соседних командах основанием
# стояло «настоящие записи настоящих людей»; оно неверно. Но это НЕ основание
# снимать защиту: неверна была причина, а не защита (DRF-2517; для нас —
# docs/CURRENT_DECISIONS_2026-09-16.md §77 п.57, вне этого репозитория).
...
class Command(BaseCommand):
    ...
    def _refuse_protected(salons: list[dict], protected: set[str]) -> None:
        ...
        Пилотный ``formula-tela`` несёт настоящий прайс настоящего салона
        (основание — у ``PROTECTED_SLUGS``). Совпадение слага означало бы,
        что сид дописывает мастеров и услуги в боевой салон — молча и
        необратимо.
        """
