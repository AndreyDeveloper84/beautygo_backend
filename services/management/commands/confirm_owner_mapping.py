...
``confirm_seeded_links`` держит ``formula-tela`` в ``PROTECTED_SLUGS``: это
боевой пилот с настоящим прайсом настоящего салона, и правило про
демонстрационный набор к нему не применяется (основание — у
``seed_demo_salons.PROTECTED_SLUGS``, DRF-2517). Здесь — другой случай и другое основание: владелец продукта
