...
class ResolvedService:
    ...
    #: Есть ли у слоя вообще место, куда мог бы лечь ответ.
    #:
    #: Отделено от ``requires_health_check`` намеренно. Трёхзначного поля
    #: не хватает: `None` в каноническом слое значит «колонка есть, никто
    #: не ответил» — это работа для очереди разметки; `None` в легаси-слое
    #: значит «колонки нет» — работы за ним не стоит никакой. Сложенные в
    ...
    *,
    ...
    if service is not None:
        ...
        return ResolvedService(
            ...
            #
            # §100 (10.09.2026): владелец подтвердил fail-closed и дал
            # состоянию собственное имя. `answerable=False` — это не
            # «строже», а точнее: отказ поедет с кодом NOT_APPLICABLE, и
            # очередь разметки этот слой не увидит, потому что размечать
            # его никто не будет — маркетплейс поедет через DRF-1622.
            requires_health_check=None,
            health_check_answerable=False,
        )
