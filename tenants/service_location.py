...
def tenant_has_no_masters(tenant) -> bool:
    """Нет ли у тенанта ни одного мастера — **условие**, без действия.

    Место оказания услуг существует, чтобы к нему привязывали мастеров
    (``SpecialistProfile.works_at``); тенанту без мастеров место не служит
    никому. «Настоящий салон или служебный тенант» схема сегодня не
    различает: ``Tenant.Kind`` знает только ``SALON`` и ``SOLO``, умолчание —
    ``SALON``. Поэтому критерий здесь — **свойство**, а не имя: список
    известных служебных слугов был бы тем самым «именем производной»,
    против которого предостерегает докстринг ``Kind``.

    Действия здесь нет намеренно. Вызывающих двое, и спрашивают они
    по-разному: команда переноса требует явного флага, форма админки не
    проходит валидацию. Затащи мы действие внутрь — предикат не смог бы
    обслужить обоих (``tenants/tests/test_service_tenant_place_guard_2030.py``).

    Счёт через ``_base_manager`` — как у соседней команды триажа
    (``propose_master_locations.py:92``): менеджер по умолчанию сегодня
    обычный, но появись у него фильтр, счёт молча обнулился бы **у всех**
    тенантов, и сторож стал бы спрашивать подтверждение на каждом салоне.
    Импорт внутри функции — ``users.models`` импортирует ``tenants``.

    **Названный предел.** У служебного тенанта мастеров нет, и по этой же
    причине сегодня правы ``users/deletion_executor.py:732`` и
    ``users/personal_data_api.py:160``, читающие «не соло → салон». Одно
    свойство держит две защиты: привязка мастера к служебному тенанту снимет
    обе разом, и подтверждением это не лечится — вопрос задаётся при нуле
    мастеров. Снимает предел только явный признак в схеме.
    """
    return tenant is not None and tenant_master_count(tenant) == 0


def tenant_master_count(tenant) -> int:
    """Сколько мастеров у тенанта — число для отказа, тем же счётом.

    Отказ обязан печатать число: «мастеров 0» отличает отказ по делу от
    отказа по ошибке. Но считать это число у вызывающего значило бы завести
    **вторую копию условия** — а разойдутся копии молча, как разошлись бы
    ``participates_in_distance`` и ``participating_place_q``, если бы их не
    держал общий тест. Поэтому и признак, и число берутся отсюда.
    """
