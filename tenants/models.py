...
class Tenant(models.Model):
    ...
    # DRF-2420 — демонстрационность как СВОЙСТВО, а не как состояние.
    #
    # Демо-салоны сеялись за тремя замками (`seed_demo_salons`):
    # `is_active=False`, `SpecialistProfile.status=PENDING`,
    # `is_booking_enabled=False`. Эти замки И БЫЛИ признаком
    # демонстрационности — другого не существовало, и `--activate` снимает все
    # три разом. Владелец держит демо живым, чтобы показывать интерфейс,
    # значит замки открыты, значит по прежней схеме демо неотличимо от
    # боевого салона. Отдельное поле нужно именно поэтому: «выключен» и «не
    # для клиента» — разные утверждения, и первым нельзя выразить второе.
    #
    # Ортогонально `kind` (салон/соло) и `is_active` (живой/выключенный):
    # демонстрационным бывает любой вид, и он остаётся ЖИВЫМ.
    is_demo = models.BooleanField(
        default=False,
        db_index=True,
        help_text=(
            "Демонстрационный салон: заведён для показа и отработки "
            "интерфейса. Остаётся живым и бронируемым, но обычному клиенту "
            "не показывается — видит только личность с "
            "`User.is_test_persona`. Читать этот флаг напрямую нельзя: "
            "видимость выражает один предикат `users.sellable`."
        ),
    )
    ...
    def clean(self) -> None:
        ...
        # Slug не меняется после создания: по нему бот привязывает салон,
        # по нему ходит X-Tenant, к нему привязан салонный бот. Сверка —
        # с тем, что записано в базе, а не с объектом в памяти. Ключ `slug`
        # безопасен для форм: на форме изменения поле только для чтения и
        # измениться не может, а там, где оно есть, ключ существует.
        # Предел: save()/update() из кода идут мимо clean().
        if not self._state.adding:
            ...
            if stored is not None and stored != self.slug:
                raise ValidationError({
                    "slug": (
                        "Slug салона не меняется после создания: по нему "
                        "салон привязан в боте и в заголовке X-Tenant."
                    ),
                })
