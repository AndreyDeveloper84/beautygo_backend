...
def salon(tenant, category):
    return SalonService.objects.create(
        ...
        duration_minutes=60,
        # Салон ОТВЕТИЛ «скрининг не нужен». До миграции 0018 сказать это
        # было нечем: поле было двузначным, и `False` означал «строку
        # создали». Теперь это ответ, и гейт пути записи такую услугу
        # пропускает — что и позволяет тесту проверять свой предмет
        # (сохранение брони), а не медицинский гейт.
...
class TestMarketplaceRegression:
    @pytest.mark.xfail(
        strict=True,
        reason=(
            "Инвариант AMD-019 (ровно одна типизированная ссылка) на "
            "маркетплейсном слое проверить НЕЧЕМ: у легаси-`Service` нет "
            "признака здоровья ни на нём, ни над ним; резолвер отдаёт для "
            "него `None` («не знаем»); гейт пути записи на `None` "
            "закрывается, и бронь не создаётся. Слой пуст — 0 строк, "
            "0 активных, замер боевого пилота 10.09.2026 03:41 UTC. "
            "Сторож оставлен `xfail`, а НЕ переписан и не удалён: его "
            "предмет — форма хранения, а не здоровье, и переписать его в "
            "«бронь отказывается» значило бы подменить сторож формы "
            "сторожем гейта — ослабление, выглядящее как работа. "
            "Если легаси-слой оживят, снимите `xfail` и почините ОДНО ИЗ "
            "ДВУХ: либо заведите признак здоровья у легаси-слоя, либо "
            "перенесите этот сторож на канонический слой. Не выбирайте "
            "вслепую — оба выхода настоящие."
        ),
    )
