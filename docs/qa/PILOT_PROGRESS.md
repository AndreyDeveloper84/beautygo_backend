...
```
Код:     █████████████░░░░░░░░░░░░░  90% done (196/219 SP)
         ░░░░████░░░░░░░░░░░░░░░░░░  16% in-progress (29 SP)
         ░░░░░░░░░░████████████░░░░  40% pending (73 SP)
```

- **Done:** 196 SP · **In-progress:** 29 SP · **Pending (код):** 57 SP · **Pending (внешнее):** 16 SP
- Осталось дней: **25** (пилот 15.08). Сделано за дни 1–2: 78 SP.
...
| Прогон покрытия на staging | 1 | ✅ (58/58, оркестратор 2026-07-21) |
...
| booking.no_show consumer (AMD-018) | 2 | W3 | ✅ |
...
- **2026-07-22 (день 5, поздний вечер):** **Click-through W4 ЗАКРЫТ полностью** — все шаги на staging: auth initData → 58 услуг/4 мастера/маппинг → 66 слотов → create(false) confirmed → запись в list → дубль (идемпотентный тот же id) → cancel → proxy cancelled по round-trip → слот освободился → create(true) awaiting_payment. Три эскалации W4 сняты кодом/данными: MasterService 232 + WorkingHours бота (данные staging), `specialist_id` = CatalogMaster.id (`04d175c`), read-модель через RemoteBookingProxy (`5c0096f`). FE-адаптация cancel UX (`ff43b20`). Тестовые записи почищены (6/7; одна — прошедший визит, безвредна). Осталось: денежный e2e с тест-картой (awaiting_payment → webview → captured).
...
- **2026-07-20 (день 3):** +1 SP — W4 отображение привязанной карты в dev `427a202` (три честных состояния: карта/consent-гейт/placeholder). Денежный UX мастера полностью закрыт. Done: 193/219 (88%).

- **2026-07-20 (день 3):** +1 SP — W4 pay-debt CTA в dev `add8242`: денежный UX мастера сквозной (C2 → привязка → C3 → погашение). Тесты FE: 164/164. Done: 192/218 (88%).
