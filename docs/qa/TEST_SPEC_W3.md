...
- **➕ Пробел G-1:** клиент не передаёт `payment_required`. Нужны: route-table расширить проверкой тела create; тест `test_create_without_prepayment` → Ayla отвечает CONFIRMED без Payment (acceptance §10.1 bot-path); UI-флаг W4 зависит от этого.
...
- **Отклонения (🔶 на решение оркестратора):** фактически — template-slug exact + normalized-name, **без** (category_slug,name)-пары, duration-tiebreaker, «»-stripping и mapping file; отчёт только в stdout (нет артефакта). Перед флипом: ➕ прогон на каталоге Пензы + coverage-отчёт оркестратору (go/no-go).

## W3-AC-04. Eventbus ingest (§5, AMD-007/008)
- **Ожидаемое:** конверт 10 полей; дедуп по `event_id` (повтор → handled once); unknown `event_version` → 422 + DLQ; unknown name → 422 + DLQ; billing-топики в ALLOWED.
...
- **🔶 D-2:** планируется также T−2h (сверх §7) — решение оркестратора.
