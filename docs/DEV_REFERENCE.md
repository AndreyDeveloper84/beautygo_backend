...
### Узел на миграцию данных

**Если миграция правит данные, а не схему, откатывать состояние базы не нужно
вовсе** — достаточно завести строки живыми моделями и позвать её функцию
напрямую:
...
Так проверяется то, что в такой миграции и может быть неверным: **какие строки
она отбирает**. Что миграция вообще применяется и что зависимость названа
верно, проверяет `migrate` на чистой базе в CI.

**Чего делать не надо, и почему** (стоило двух заходов на DRF-2406): откатывать
`services` на предыдущий номер и брать исторические модели через
`MigrationExecutor`. Назад откатывается **одно приложение, а колонки остальных
остаются**: историческая модель тенанта не знала поля `kind`, которое база
требует `NOT NULL`, и узел падал на фикстуре, а не на предмете.

**Литералы в миграции дублировать намеренно.** Миграция не вправе зависеть от
кода приложения — его переименуют, а она должна остаться накатываемой. Дубль
неустраним, поэтому его стерегут узлом (`RULE == MASTER_SELECT_RULE`), а не
устраняют импортом.
