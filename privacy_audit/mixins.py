...
operations stop. Everything else proceeds, because our outage must not stop
a person acting on their own data — and, there being no queue (owner D1,
12.09.2026), the loss is an ERROR line, not a deferred row. The ordering
above still matters for those: the ERROR names what actually happened.
...
class AuditedPersonalDataAccess:
    ...
    def _journal(self, response) -> None:
        ...
        # ``record_or_lose`` owns the owner's boundary (§107 + D1): a
        # disclosing or destroying operation that cannot be journalled raises
        # and is refused by the caller below; anything else — and every
        # denial — proceeds with the loss written at ERROR, there being no
        # queue to carry the row (owner D1, 12.09.2026).
        stored = record_or_lose(
        ...
        if stored != "written":
            # The ERROR with the composition is already written by
            # record_or_lose; this WARNING adds the route for the operator.
            logger.warning(
                "privacy_audit.record_%s path=%s operation=%s result=%s",
                stored, request.path, operation,
