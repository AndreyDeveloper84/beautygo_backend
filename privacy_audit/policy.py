...
FAIL_CLOSED_OPERATIONS: frozenset[str] = frozenset({
    ...
    # DRF-1813 (M21) — удаление фото мастера (аватар, портфолио): «удаление»
    # персданных буквально, наравне с уже закрытым DELETE, а не по аналогии.
    # Решение главного окна 15.09: журнал недоступен — 503, ничего не
    # удалено (файл уходит из хранилища только после фиксации), повтор
    # безопасен. Загрузка и запись полей профиля сюда не входят — как
    # WRITE_CONTEXT.
    _Op.DELETE_MEDIA,
})
