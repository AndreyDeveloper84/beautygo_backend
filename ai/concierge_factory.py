...
#: Что промпт получает вместо кандидатов, когда их нет.
#:
#: Роль бота по решению владельца — **language rendering**: он облекает в
#: слова УЖЕ ПРИНЯТОЕ состояние, а не решает, что сказать. Значит в
#: промпт идёт машинно-читаемое имя состояния и список разрешённых
#: продолжений, а формулировку модель строит сама.
#:
#: **Почему имя `NO_CANDIDATES`, а не `NO_VERIFIED_CANDIDATES`.**
#: Второе — состояние резолвера, и оно утверждает, что проверка связей
#: была и не нашла подтверждённых. Сегодня промпт собирается мимо
#: резолвера: пусто здесь значит «движок после фильтров не вернул
#: никого», и проверки не было вовсе. Назвать это `NO_VERIFIED_
#: CANDIDATES` значило бы приписать системе шаг, которого она не
#: сделала, — та же подмена, что «связи ещё нет» против «связи не
#: будет» (§93).
#:
#: **Это требование, а не осторожность автора.** Владелец закрыл В-13
#: дословно: состояние НЕ ВПРАВЕ утверждать, что стадия конвейера
#: произошла, если она не происходила, и `NO_CANDIDATES` не «исправлять»
#: на `NO_VERIFIED_CANDIDATES` до того, как канонический резолвер и гейт
#: маппинга действительно отработают.
#:
#: Настоящее состояние резолвера приедет сюда, когда промпт пойдёт через
#: него (1б, понижение движка до стадии конвейера). Канал для этого —
#: уже здесь; наполнять его правдой раньше времени нельзя.
#:
#: Молчание запрещено отдельно: владелец сказал, что отсутствие
#: рекомендации не равно отсутствию пользы. Поэтому продолжения
#: перечислены, а не подразумеваются.
EMPTY_CANDIDATES_PROMPT_STATE = (
    "STATE=NO_CANDIDATES\n"
    "Подходящих кандидатов нет. Конкретных мастеров и услуг называть "
    "нельзя — их нет в этом контексте.\n"
    "Разрешённые продолжения: показать каталог | уточнить запрос | "
...
def to_core_specialist_context(
    ...
    *,
    ...
    """
    # ПОРЯДОК ДВИЖКА СЮДА НЕ ПРОХОДИТ (DRF-1628/1630).
    #
    # `local.candidates` приходит отсортированным движком по composite —
    # рейтинг, расстояние, цель. Ядро печатает список в промпт как есть
    # (`render_summary_text`), а модель читает сверху вниз и первого
    # называет лучшим. То есть ранжирование доходило до речи **не
    # числами, а позицией**: ни рейтинга, ни балла в промпте нет, и
    # поиском по «score» этого было не найти.
    #
    # Владелец (§DRF-1628) отнял у движка не право считать, а право быть
    # конечным ответом. Значит его порядок не вправе быть и порядком в
    # промпте. Здесь список пересобирается в НЕЙТРАЛЬНЫЙ порядок.
    #
    # Почему по имени, а не по `id`: обе величины несемантичны, но
    # алфавит человек читает как список, а не как рейтинг. UUID-порядок
    # выглядел бы случайным и провоцировал бы вопрос «почему этот
    # первый», у которого нет ответа. `id` остаётся тай-брейком, чтобы
    # порядок был воспроизводим между репликами при совпадении имён.
    candidates = [
        SpecialistCandidate(
            id=c.id,
            name=c.display_name,
        ...
        for c in sorted(
            local.candidates,
            key=lambda c: (c.display_name.casefold(), str(c.id)),
        )
    ]
    context = build_specialist_context_from_candidates(
        candidates, tenant_id=tenant_id,
    )
    if candidates:
        return context
    # Пустой случай ядро рендерит фразой «нет активных мастеров —
    # записаться можно только через менеджера». Она про АКТИВНОСТЬ и
    # ведёт человека к менеджеру, а состояния не называет. Модель,
    # получив её, дальше сочиняет сама.
    return replace(context, summary_text=EMPTY_CANDIDATES_PROMPT_STATE)
