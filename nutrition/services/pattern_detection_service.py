...
#
# Столбцы ``daily_*`` nullable (§103, миграция 0018): ``NULL`` — ориентира
# нет. Внутри этого модуля отсутствие переводится в ``0.0`` осознанно:
# все вызывающие проверяют знаменатель на положительность, а наружу
# уезжает паттерн или его отсутствие, не само число. ``if profile.x``
# ловит и ``None``, и ноль, оставшийся у строк до очистки командой.
#
...
def _profile_goal_water(profile: NutritionProfile | None) -> float:
    """Ориентира по жидкости нет ни у кого — 0.0 значит «нет».

    Читалось ``profile.daily_water_ml``. Значение в этом столбце — выход
    формулы 30 мл × вес (+300/+700), которую владелец снял 09.09.2026
    (§82). У существующих клиентов столбец ещё заполнен, поэтому чтение
    снято вместе с записью: пока в базе лежит выход снятой формулы,
    прочитать его значит эту формулу применить.

    Ноль здесь безопасен и остаётся нулём, а не ``None``: все вызывающие
    внутри модуля уже проверяют знаменатель на положительность, и наружу
    из паттернов это число не уезжает — уезжает сам паттерн или его
    отсутствие.
    """
    return 0.0
