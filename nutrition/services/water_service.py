...
class WaterAggregate:
    """Факт и ориентир воды одного дня.

    Кнопочный трекер (``WaterLog``, этот модуль) ориентира не ставит —
    ``None``. Сводка дня (DRF-2257) кладёт сюда ДЕЙСТВУЮЩИЙ ориентир из
    ``water_entry_service.fluid_target_ml`` — того же, что у экрана воды
    (§85 п.4 под ``fluids_confirmed``); без него — ``None``, и сериализатор
    выкидывает ключ. Числа здесь не подставляются.
    """
