...
class DeletedFoodLog(models.Model):
    ...
    (``users.deletion_executor``), «забудь всё» — тоже
    (``users.forget_all_catalog``, DRF-2214).
    """
...
class NutritionProfile(models.Model):
    ...
    # Когда человек НАЗВАЛ свой тип питания. NULL — не называл: в столбце
    # выше лежит либо умолчание, либо прежняя свободная строка (§77 п. 7).
    #
    # Почему не пометкой ``legacy_default_inputs``, как у темпа и активности:
    # там прежнее значение узнаётся ПО САМОМУ ЗНАЧЕНИЮ (1.4, «moderate») —
    # оно и есть след умолчания. Здесь же любое слово из списка одинаково
    # может быть и прежней свободной строкой, и настоящим ответом; пометка
    # по значению после выхода вопроса метила бы каждого ответившего и
    # переспрашивала бы человека, который уже ответил. Факт ответа
    # различает их сам и не требует ни команды, ни срока.
    diet_answered_at = models.DateTimeField(null=True, blank=True, default=None)
    ...
    class TargetsSource(models.TextChoices):
        ...
        #: Посчитано Ayla, человеком НЕ подтверждено (решение владельца
        #: 11.09.2026 §5.1: «результат становится привычкой/шагом только
        #: после подтверждения пользователя»). Это состояние источника,
        #: а не флаг рядом с ним: флаг «подтверждено» при источнике
        #: ``ayla_calculated`` дал бы две правды об одном числе.
        #: Предложение показывается человеку как предложение и в
        #: расчёты «осталось»/оценки не входит — наравне с отсутствием.
        AYLA_PROPOSED = "ayla_proposed", "Предложено Ayla, не подтверждено"
        AYLA_CALCULATED = "ayla_calculated", "Рассчитано Ayla, подтверждено"
    ...
    targets_confirmed_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=(
            "Когда человек подтвердил предложенный ориентир (§5.1). NULL — "
            "не подтверждал: ориентир либо предложен (ayla_proposed), либо "
            "его нет, либо он поставлен до введения подтверждения."
        ),
    )

    # ── Происхождение ПО ВИДАМ (DRF-1929, F1(б)) ────────────────────────
    #
    # Решение владельца: происхождение хранится по каждому виду отдельно;
    # одно общее ``targets_source`` на набор решению НЕ соответствует.
    #
    # Почему это выравнивание, а не нововведение: соседнее поле
    # ``targets_method_versions`` УЖЕ по видам ({"calories": …,
    # "fluids": …}) и по той же причине — «у калорий и жидкости методики
    # разные и меняются порознь (§85)». Происхождение за этой гранью
    # просто не пошло, и из-за этого ручная правка ОДНОГО вида меняла
    # подпись ВСЕГО набора.
    #
    # Что это чинит на самом деле: не «качество провенанса», а ПОТЕРЮ
    # ЧИСЛА. Пока подпись одна, справочная вода после ручных калорий
    # становилась бы «числом человека», которого он не называл, — и
    # ``manual_targets_service`` гасил её в NULL, чтобы не соврать. С
    # раздельной подписью гасить нечего: вода остаётся со своим
    # происхождением.
    #
    # ``null=True`` и БЕЗ ``default``, намеренно: колонка с умолчанием
    # проставила бы значение каждой существующей строке прямо в DDL, то
    # есть объявила бы по-видовое происхождение там, где его никто не
    # устанавливал — ровно то, от чего отказалась ``0017`` («утверждение,
    # которого мы не можем проверить»). NULL здесь — не оттенок пустоты,
    # а ТРЕТЬЕ состояние: «по видам не устанавливалось».
    #
    # Существующим строкам имя даёт ``0023`` — решение владельца
    # 16.09.2026: ``unknown_legacy`` тому виду, у которого число есть,
    # ``none`` тому, у которого его нет; старый ``targets_source`` НЕ
    # копируется (он про набор, а не про вид). После ``0023`` ``NULL``
    # остаётся только у строк, заведённых позже и ещё не писавших
    # происхождение.
    calories_source = models.CharField(
        ...
        blank=True,
        help_text=(
            "Происхождение ориентира по КАЛОРИЯМ. NULL — по видам не "
            "устанавливалось (строка заведена до DRF-1929); это не то же "
            "самое, что `none` («ориентира нет»)."
        ),
    )
    fluids_source = models.CharField(
        max_length=24,
        choices=TargetsSource.choices,
        null=True,
        blank=True,
        help_text=(
            "Происхождение ориентира по ЖИДКОСТИ. NULL — по видам не "
            "устанавливалось (строка заведена до DRF-1929); это не то же "
            "самое, что `none` («ориентира нет»)."
        ),
    )
    calories_confirmed_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=(
            "Когда человек подтвердил ориентир по калориям (§5.1). "
            "Подтверждение — по виду, не одной кнопкой на весь набор."
        ),
    )
    fluids_confirmed_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text=(
            "Когда человек подтвердил ориентир по жидкости (§5.1). "
            "Подтверждение — по виду, не одной кнопкой на весь набор."
        ),
    )
...
class SavedMeal(models.Model):
    ...
    Стирают её erasure-исполнитель вместе с личностью
    (``users.deletion_executor``, реестр ``DELETE``) и «забудь всё»
    (``users.forget_all_catalog``, DRF-2214) — включая мягко удалённые:
    «скрыта» не значит «забыта».
