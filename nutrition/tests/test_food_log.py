...
class TestScanPath:
    ...
    def test_scan_with_partial_null_totals_returns_food_not_recognized(
        ...
        # чисел. Числа такая запись уже не получит: правка порции их не
        # пересчитывает (food_log_edit_service не ищет блюдо заново), и
        # ручки «досчитать» в каталоге нет — названо как предел, а не как
        # обещание.
        scan = FoodScan.objects.create(
