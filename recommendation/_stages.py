...
class StagePolicy:
    ...
    #: §10.4 вариант (а): «на пилоте считать каталог VERIFIED». Только
    #: явным решением владельца, и каждый затронутый кандидат получает
    #: свидетельство `UNSUBSTANTIATED` с `source_ref="pilot_override"` —
    #: разрешённое исключение обязано быть видно в свидетельстве, иначе
    #: через месяц никто не отличит «проверено» от «разрешено на время».
    mapping_override_enabled: bool = False
    ...
    def from_settings(cls) -> "StagePolicy":
        ...
        Здесь исправляется расхождение, заведённое внутри самой границы.
        Домашний экран собирал политику у себя и читал
        ``RECOMMENDATION_PILOT_MAPPING_OVERRIDE``; HTTP-проекция звала
        `resolve()` без политики и получала жёсткое умолчание. Одна
        политика — два значения в одном процессе, ровно тот разрыв,
        ради устранения которого эпик и существует, только в политике,
        а не в ранжировании.

        Сегодня оба значения совпадают (`False`), и поэтому расхождение
        невидимо. Оно станет видимым **в момент ответа владельца по
        §40.4 п.1**: флаг включат, домашний экран оживёт, бот останется
        тёмным — и это будет выглядеть так, будто включение сработало.
        Худший вид дефекта: тот, который проявляется ровно тогда, когда
        на него перестают смотреть.

        Поэтому настройку читает резолвер. У поверхностей больше нет
        места, где политику можно собрать по-своему, — по той же причине,
        по которой у них нет места, где можно упорядочить кандидатов.
        Явно переданная политика по-прежнему сильнее: тест обязан уметь
        назвать её без правки настроек.
        """
...
def _mapping_admission(facts: CandidateFacts, policy: StagePolicy) -> tuple[bool, EvidenceItem | None]:
    ...
    if policy.mapping_override_enabled:
        return True, EvidenceItem(
            kind=EvidenceKind.CAPABILITY_MAPPING,
            value=facts.mapping_status,
            strength=EvidenceStrength.UNSUBSTANTIATED,
            origin=EvidenceOrigin.CURATED_KNOWLEDGE,
            source_ref="pilot_override",
        )
    return False, None
