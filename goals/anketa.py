...
ANKETA_STEPS: tuple[AnketaStep, ...] = (
    AnketaStep(
        ...
        # DRF-2177 — макет C03 (DRF-1178): «Не знаю» доступно ВСЕГДА, не
        # только там, где человек «может не знать» (DRF-1747 ставил его на
        # одном шаге). Решение владельца §60: как в макетах.
        escape=True,
    ),
    AnketaStep(
        key="feeling",
        ...
        # DRF-1759 — макет C02.2: результатов может быть несколько
        # («Отдохнувшей» и «Спокойнее» разом), «Продолжить» при ≥1.
        # Мультивыбор — именно результат, не область: ``area`` остаётся
        # одним ответом. Значение шага по-прежнему никто не читает
        # (замер §5.3), поэтому массив не меняет подбор.
        mode=MODE_MULTI,
        # DRF-1747 — вопрос о самочувствии: здесь человек может не знать.
        escape=True,
    ),
    # DRF-2173 — срок цели («До 1 ноября 2026» на главном экране). Последний
    # и НЕОБЯЗАТЕЛЬНЫЙ: «без срока» — умолчание и штатный ответ, после него
    # проход завершается как после любого последнего шага; блокировать поток
    # шаг не может (C-2). Свободный текст («до 1 ноября») разбирает
    # ``goals/deadline.py`` детерминированно; отказы — 400 словами. Подписей
    # со словами «дата»/«время» здесь нет намеренно — сторож границы C03
    # (DRF-1751) держит анкету без тем расписания, и срок цели он от даты
    # записи по словам не отличит. Значение шага подбор не читает
    # (``rule_ids`` пуст → пометка «на подбор не влияет» — честно): оно
    # уходит в ``ClientGoal.target_date`` и рисуется на карточке цели.
    AnketaStep(
        key="deadline",
        prompt="К какому сроку хочешь? Можно пропустить",
        options=(
