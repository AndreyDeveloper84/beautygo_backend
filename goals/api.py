...
class GoalSelectView(APIView):
    ...
    def _answer_anketa(
        ...
        *,
        ...
        # задал первый сужающий шаг под неё (`build_decision_context`).
        # Ожидаемый шаг считается так же, как считал документ: шаг цели
        # отвечен самой целью. Без цели и без прохода — как прежде, первым
        # ждётся цель.
        active_goal = (
        ...
        expected = (
            anketa.next_step({anketa.GOAL_STEP_KEY})
            if run is None and active_goal is not None
            else next_anketa_step(run)
        )
        ...
        if run is None:
            ...
            if active_goal is not None and run.goal_id is None:
