...
Почему None — не катастрофа и почему пилот не ломается
------------------------------------------------------

Каноническая таксономия ``ServiceCategory`` заводится **без салона**
(``seed_canonical_catalog``: «2-level taxonomy, tenant-null / global»).
Именно её имена — «Маникюр», «Уход», «Стрижка» — и покрывают почти весь
замер DRF-1461. Они читаются всегда, потому что не принадлежат никакому
салону, а значит и решением «по чужим строкам» не являются.

Салонными являются ``SalonService`` — прайс конкретного салона. Вот их
при неизвестном салоне мы и перестаём читать, и ровно на них указывает
DRF-1455: «услуга, которой в салоне A нет, но которая есть в салоне B».

Клиент-прокси, созданный ботом, до первой записи не привязан ни к
какому салону (связь выдаётся на первой записи, #1014). Пока бот не
начнёт слать ``X-Tenant`` на goal-вызовах, такой клиент судит по общей
таксономии — то есть по тому же, по чему судил и до правки, минус чужие
прайсы. Вопрос владельцу про заголовок на стороне бота вынесен
отдельно: без него салон клиента до первой записи знать неоткуда.

Сознательно НЕ делается: подстановка «единственного салона
развёртывания». Салонов в системе всегда минимум два — миграция
``tenants/0003`` заводит ``formula-tela`` и ``ayla-marketplace``, — так
что такой вывод был бы не выводом, а лотереей.
"""
...
def goal_tenant_id(goal, *, client: User | None = None) -> uuid.UUID | None:
    ...
    Прежде всего — салон самой цели. Он NULL у строк, созданных до
    DRF-1455 и не покрытых бэкфиллом, и у клиентов, чей салон не
    определяется однозначно. Для них падаем в общее разрешение по
    клиенту — чтобы старая цель не перестала распознаваться на ровном
    месте только потому, что колонка появилась позже неё.
    """
